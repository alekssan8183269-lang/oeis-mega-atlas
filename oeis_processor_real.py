...
MAX_ROWS_PER_FILE = 10000   # Жёсткий лимит строк на один файл, чтобы Excel не тупил!
...
def analyze_sequence(seq_id, seq_name, raw_sequence):
    start_compute_time = time.perf_counter()  # ЗАПУСКАЕМ СЧЕТЧИК МЕТА-ВРЕМЕНИ    
    ...
    try:
        ...
        try:
            ...
            # Для анализа триплетов нам нужно хотя бы 5 шагов (элементов бинарного профиля)  
            if len(bin_profile) >= 5:
                rule_table = {}
                is_cellular_automaton = 1.0
                
                # Сканируем триплеты и их следствие через один шаг
                ...
                if is_cellular_automaton == 1.0 and len(rule_table) > 0:
                    wolfram_rule_id = 0
                    ...
                    for idx_bit, trip in enumerate(wolfram_triplets):
                        # Если триплет встретился в ряду, берем его реальный исход.
                        # Если не встретился — ставим 0 (дефолт по Вольфраму для нереализованных шагов)                        
                        bit_val = rule_table.get(trip, 0) # Если триплет не встретился, по умолчанию 0
                        wolfram_rule_id += bit_val * (2 ** (7 - idx_bit))
                else:
                    # Если детерминизм нарушен — сбрасываем в -1 (Не автомат)
                    is_cellular_automaton = 0.0
        ...
        try:
            ...
            if n_set > 0:
                ...
                # ИСПРАВЛЕНО: Считаем реальный процент элементов, которые не являются строго целыми
                # или нарушают дискретную сетку (например, если в ряду реально затесались float)
                # non_int_count = sum(1 for x in set_elements if not float(x).is_integer())
                # set_fractional_ratio = float(non_int_count / n_set)

                # Проверяем, сколько чисел при делении на средний шаг дают остаток (дробную часть)
                mean_diff = max(1.0, np.mean(np.abs(np.diff(seq[:50]))))
                fractional_elements = sum(1 for x in set_elements if (float(x) % mean_diff) != 0)
                set_fractional_ratio = float(fractional_elements / n_set)
            else:
        ...
        # =====================================================================
        # РАЗДЕЛ 23: БЛОК САМОАНАЛИЗА И МЕТА-РЕФЛЕКСИИ КОДА (РОВНО 10 МЕТРИК)
        # =====================================================================
        try:
            # 1. Метрика Вычислительного Затыка (Compute Stall Index)
            # Сколько времени ушло на этот ряд относительно базового тика (0.001 сек)
            elapsed_time = time.perf_counter() - start_compute_time
            compute_stall_index = float(elapsed_time / 0.001)

            # Перечень колонок для проверки на аварийные нули (в твоем стиле)
            meta_check_vars = [
                causal_emergence, renyi_ent, tsallis_ent, perm_ent_l2, perm_ent_l3,
                betti_0_small, betti_1_large, minkowski_box_dim, convex_hull_volume,
                prime_factor_density, coprimality_rate, poly_degree,
                lz_complexity_ratio, ieee754_mantissa_entropy, von_neumann_entropy,
                poincare_return_mean, laminarity_score, ring_perfection_score,
                is_cellular_automaton, invariant_perfection
            ]
            
            # 2. Коэффициент Срабатывания Заглушек (Sieve Fallback Rate)
            # Процент датчиков, которые упали в дефолтные аварийные значения
            zeros_count = sum(1 for v in meta_check_vars if v == 0.0 or v == 1.0 or v == 0.5)
            sieve_fallback_rate = float(zeros_count / len(meta_check_vars))

            # 3. Энтропия Сжатия Признаков (Feature Space Entropy)
            # Насколько разнообразны получившиеся ответы наших главных датчиков
            safe_meta_vars = np.array([abs(float(v)) for v in meta_check_vars if np.isfinite(v)])
            if len(safe_meta_vars) > 1 and np.sum(safe_meta_vars) > 0:
                meta_p = safe_meta_vars / np.sum(safe_meta_vars)
                feature_space_entropy = float(-np.sum(meta_p * np.log2(meta_p + 1e-12)))
            else:
                feature_space_entropy = 0.0

            # 4. Индекс Когнитивного Диссонанса Датчиков (Sensor Dissociation Score)
            # Разница между линейным трендом Спирмена и волновым ситом Шварца
            sensor_dissociation = float(abs((spearman_trend_correlation if 'spearman_trend_correlation' in locals() else 0.0) - (lwle_bipolar_sieve_density if 'lwle_bipolar_sieve_density' in locals() else 0.0)))

            # 5. Глубина Нейро-Стриппинга (Data Clipping Depth)
            # Оценка урезания: если длина ряда изначально была больше 40 чисел
            data_clipping_depth = float(max(0.0, (n_len - 40) / max(1, n_len)))

            # 6. Алгебраический Процент Уверенности (Algebraic Confidence Rate)
            # Насколько структура множества близка к чистому кристаллическому кольцу
            algebraic_confidence = float(abs((ring_perfection_score if 'ring_perfection_score' in locals() else 0.5) - 0.5) * 2.0)

            # 7. Коэффициент Кроссбар-Резонанса (Crossbar Resonance Index)
            # Пересечение волновой плотности мантиссы и сложности таблицы переходов Вольфрама
            crossbar_resonance = float(abs((ieee754_mantissa_entropy if 'ieee754_mantissa_entropy' in locals() else 0.0) * (ca_complexity if 'ca_complexity' in locals() else 0.5)))

            # 8. Энергетическая Вариация Ландауэра (Dynamic Heat Counterpart)
            # Теоретическое тепловыделение при схлопывании Лапласиана графа видимости
            dynamic_heat_landauer = float(abs((nvg_graph_energy if 'nvg_graph_energy' in locals() else 0.0) * 1.38e-23 * 298.15))

            # 9. Маркер Интуического Схлопывания (Aha-Moment Trigger)
            # Сигнал озарения: если инвариант закона идеален при высоком марковском шуме
            if micro_noise > 0.5 and invariant_perfection > 0.7:
                aha_moment_trigger = 1.0
            else:
                aha_moment_trigger = 0.0

            # 10. Фрактальный Индекс Эволюции Личности Кода (System Mutation Pace)
            # Как сильно волновой квантор сместил баланс сингулярного спектра SVD
            system_mutation_pace = float(abs((lwle_spiral_quantor if 'lwle_spiral_quantor' in locals() else 0.5) - (svd_entropy_score if 'svd_entropy_score' in locals() else 0.5)))

        except:
            compute_stall_index, sieve_fallback_rate, feature_space_entropy = 0.0, 1.0, 0.0
            sensor_dissociation, data_clipping_depth, algebraic_confidence = 0.0, 0.0, 0.0
            crossbar_resonance, dynamic_heat_landauer, aha_moment_trigger, system_mutation_pace = 0.0, 0.0, 0.0, 0.0
        ...
        return {
            ...
            # =====================================================================
            # БЛОК САМОАНАЛИЗА И МЕТА-РЕФЛЕКСИИ КОДА (РОВНО 10 КОЛОНОК)
            # =====================================================================
            "Meta_Индекс_Вычислительного_Затыка": safe_num(compute_stall_index, 4),
            "Meta_Коэффициент_Срабатывания_Заглушек": safe_num(sieve_fallback_rate, 4),
            "Meta_Энтропия_Сжатия_Признаков": safe_num(feature_space_entropy, 4),
            "Meta_Диссонанс_Датчиков_Тренда": safe_num(sensor_dissociation, 4),
            "Meta_Глубина_Нейро_Стриппинга_Обрезки": safe_num(data_clipping_depth, 4),
            "Meta_Алгебраический_Процент_Уверенности": safe_num(algebraic_confidence, 4),
            "Meta_Коэффициент_Кроссбар_Резонанса": safe_num(crossbar_resonance, 4),
            "Meta_Энергия_Ландауэра_Тепловыделение": safe_num(dynamic_heat_landauer, 25),
            "Meta_Маркер_Озарения_Aha_Moment": safe_num(aha_moment_trigger, 0),
            "Meta_Индекс_Мутации_Личности_Кода": safe_num(system_mutation_pace, 4)
...
if __name__ == "__main__":
    print("🛸 Инициализация Ультимативного 140-Поточного Мега-Атласа...")
    ...
    # Ищем все файлы, подходящие под маску (например, oeis_mega_atlas_40fields_part*.csv)
    base_pattern = OUTPUT_FILE.replace('.csv', '') + "_part*.csv"
    existing_parts = glob.glob(base_pattern)
    ...
    # Также проверим старый базовый файл без суффикса part, если он есть
    if os.path.isfile(OUTPUT_FILE):
        existing_parts.append(OUTPUT_FILE)
        
        
    if existing_parts:
        print(f"🕵️‍♂️ Найдена прошлая сессия! Сканируем найденные части ({len(existing_parts)} шт.) на готовые ряды...")
        total_rows_found = 0
        for part_file in existing_parts:
            try:
                # Читаем только колонку ID из каждой части для экономии ОЗУ
                existing_df = pd.read_csv(part_file, sep=";", usecols=["ID"], encoding="utf-8-sig")
                # Очищаем формулу Excel '="A000001"' обратно в чистый ID 'A000001'
                ids_in_part = existing_df["ID"].astype(str).str.replace('="', '', regex=False).str.replace('"', '', regex=False).tolist()
                processed_ids.update(ids_in_part)
                total_rows_found += len(ids_in_part)
                print(f"   ↳ Проверен файл: {part_file} (найдено {len(ids_in_part)} рядов)")
            except Exception as e:
                print(f"   ⚠️ Не удалось прочесть файл {part_file} ({e}), пропускаем его.")
        
        print(f"✅ Успешно загружено: {len(processed_ids)} уникальных обработанных рядов. Продолжаем без дубликатов!")
        
        # Автоматически определяем, с какого номера части возобновить запись, 
        # чтобы не затереть старые файлы при батч-записи.
        # Считаем количество строк во всех частях, чтобы понять текущий глобальный индекс.
        current_part = max(1, len(glob.glob(OUTPUT_FILE.replace('.csv', '') + "_part*.csv")))
    else:
        print(" Прошлой сессии не найдено. Начинаем тотальное сканирование базы с самого начала!")
        current_part = 1

    # Настраиваем имя файла для текущего старта
    # current_output_file = f"{OUTPUT_FILE.replace('.csv', '')}_part{current_part}.csv"
    # file_exists = os.path.isfile(current_output_file)
    ...
    # Задаем имя файла на основе вычисленного номера части
    current_output_file = f"{OUTPUT_FILE.replace('.csv', '')}_part{current_part}.csv"
    file_exists = os.path.isfile(current_output_file)
    
    # Узнаем, сколько строк УЖЕ записано в этот файл, чтобы не превысить лимит
    current_file_rows = 0
    if file_exists:
        try:
            current_file_rows = len(pd.read_csv(current_output_file, sep=";", usecols=["ID"]))
            print(f"📝 Текущий файл части {current_part} уже содержит {current_file_rows} строк. Дописываем в него.")
        except:
            current_file_rows = 0

    # Индексы для контроля автонарезки файлов и измерения скорости
    # current_part = 1
    # Базовое имя файла для первой части
    # current_output_file = f"{OUTPUT_FILE.replace('.csv', '')}_part{current_part}.csv"
    # file_exists = os.path.isfile(current_output_file)
    batch_data = []
    ...
    print("⌨️  ПОДСКАЗКА: Нажми [ПРОБЕЛ] в любой момент, чтобы поставить конвейер на ПАУЗУ.")

    # Подключаем быстрый перехватчик клавиатуры для Windows
    import msvcrt

    with gzip.open(INPUT_STRIPPED, 'rt', encoding='utf-8') as f:
        for line in f:
            # --- БЛОК ИНТЕРАКТИВНОЙ ПАУЗЫ С ПРОЦЕНТАМИ (ПРОБЕЛ) ---
            if msvcrt.kbhit(): # Проверяем, нажата ли вообще какая-то кнопка
                key = msvcrt.getch()
                if key == b' ': # Если это пробел (байт пробела)
                    # 1. Считаем время и скорость
                    elapsed_total_sec = time.time() - start_time
                    elapsed_min = int(elapsed_total_sec // 60)
                    elapsed_sec = int(elapsed_total_sec % 60)
                    
                    # Защита от деления на ноль, если нажали пробел в первую секунду
                    current_speed = round(processed_count / ((elapsed_total_sec / 60.0) + 1e-12), 1)

                    # 2. Вычисляем точный прогресс на текущий момент
                    progress_pct = round((processed_count / LIMIT_SEQUENCES) * 100, 2)
                    remaining_seqs = LIMIT_SEQUENCES - processed_count

                    print("\n🛑 || КОНВЕЙЕР ПОСТАВЛЕН НА ПАУЗУ ||")
                    print(f"📊 [СТАТУС КОНВЕЙЕРА]: Выполнено {progress_pct}% плана!")
                    print(f"📈 Обработано рядов за сессию: {processed_count} из {LIMIT_SEQUENCES} (Осталось: {remaining_seqs})")
                    print(f"⏱️  Времени прошло: {elapsed_min} мин. {elapsed_sec} сек.")
                    print(f"⚡ Текущая скорость: {current_speed} рядов/мин.")
                    print("💾 Все текущие данные в безопасности. Для возобновления снова нажми [ПРОБЕЛ]...")
                                        
                    # Запускаем бесконечный цикл ожидания второго пробела
                    while True:
                        if msvcrt.kbhit():
                            next_key = msvcrt.getch()
                            if next_key == b' ':
                                print("▶️  || КОНВЕЙЕР ЗАПУЩЕН НАДЛЕЖАЩИМ ОБРАЗОМ ||\n")
                                break
                            else:
                                print("⚠️  Нажми именно [ПРОБЕЛ], чтобы продолжить работу!")
            # ------------------------------------------
            ...
            if total_processed_global % 200 == 0:
                ...
                print(f"⏱️ Скорость: {speed} рядов/мин. Всего пройдено за сессию: {total_processed_global}")
            ...
            if len(batch_data) == BATCH_SIZE or processed_count >= LIMIT_SEQUENCES:
                ...
                # Записываем в текущую часть файла (current_output_file) вместо глобального OUTPUT_FILE
                if not file_exists:
                    df_batch.to_csv(current_output_file, index=False, sep=";", encoding="utf-8-sig", mode='w')
                    file_exists = True
                else:
                    df_batch.to_csv(current_output_file, index=False, sep=";", encoding="utf-8-sig", mode='a', header=False)
                
                total_in_file = len(processed_ids) + newly_processed_count                
                print(f"💾 [БАТЧ ЗАПИСАН В CSV] Файл: {current_output_file}. Пройдено: {processed_count}/{LIMIT_SEQUENCES} рядов.")
                print(f"💾 [БАТЧ ЗАПИСАН] Новых: {newly_processed_count}/{LIMIT_SEQUENCES}. Всего в Атласе: {total_in_file} рядов.") 
                batch_data = []
                time.sleep(0.3) # Остужаем i5
                ...
                if current_file_rows >= MAX_ROWS_PER_FILE:
                    ...
                    current_part += 1
                    current_file_rows = 0
                    # Формируем новое имя: oeis_mega_atlas_40fields_part2.csv и т.д.
                    current_output_file = f"{OUTPUT_FILE.replace('.csv', '')}_part{current_part}.csv"
                    file_exists = os.path.isfile(current_output_file) # Сброс флага для заголовков нового файла
                    print(f"📂 Следующий батч пойдет в новый файл: {current_output_file}")
