...
OUTPUT_FILE = "crypto_hype_chaos_dataset.csv"

# Обновленный набор моделей (добавлен сложный нелинейный хайп-генератор)
MARKET_MODELS = [
    "Crypto_Bull_Trend", 
    "Crypto_Bear_Crash", 
    "HFT_Market_Flat", 
    "Whale_Pump_Chaos",
    "Crypto_Hyper_Hype_Chaos"  # <- Новый ультра-нелинейный режим хайпа
]

def safe_num(val, ndigits=4):
    """Защита ячеек от автоформата Excel"""
    try: return f'="/{round(float(val), ndigits)}"' if float(val) < 0 else f'="{round(float(val), ndigits)}"'
    except: return f'="{val}"'

# =====================================================================
# ШАГ 1: ГЕНЕРАТОР МОДЕЛЕЙ ВОЛАТИЛЬНОСТИ И УЛЬТРА-ХАЙПА
# =====================================================================
def generate_crypto_market_series(model_type, length, row_idx):
    """Генерирует временные ряды, включая сложные нелинейные хаотические системы"""
    ...
    if model_type == "Crypto_Bull_Trend":
        ...
        # УЛЬТРА-НЕЛИНЕЙНОСТЬ: Динамическая система с обратной связью (генератор пузырей FOMO)
        # Имитирует разгон толпы, фазу "параболического залета" и каскадные ликвидации плеч
        price = 20.0
        velocity = 0.1       # "Ускорение" тренда
        hype_sentiment = 1.0 # Уровень хайпа в Твиттере/СМИ
        
        for t in range(length):
            # Нелинейное уравнение: хайп растет от изменения цены, а цена ускоряется от хайпа
            # Добавлена тригонометрическая циклическая неустойчивость (рыночные циклы)
            hype_sentiment += 0.05 * velocity + math.sin(t * 0.15) * 0.2
            hype_sentiment = max(0.1, min(hype_sentiment, 15.0)) # Ограничение перегрева
            
            # Параболический компонент роста + нелинейный шум (зависит от текущего хайпа)
            shock = np.random.normal(0.1 * hype_sentiment, 0.5 * (hype_sentiment**1.2))
            
            # Эффект сквиза (Short Squeeze) — если хайп запредельный, цену резко швыряет вверх
            ...
            # КРИТИЧЕСКАЯ ТОЧКА (Схлопывание пузыря / Дамп лонгов)
            # Если цена улетела слишком высоко, происходит каскад маржин-коллов
            if price > 150.0 and np.random.rand() > 0.75:
                price *= 0.4  # Моментальный обвал на 60%
                velocity = -abs(velocity) * 1.5
                hype_sentiment *= 0.2 # Выжигание хайпа в страх
                
            time_series.append(max(0.1, price))

    # Накладываем 1% биржевого шума (микро-спреды торгового стакана)
...
def analyze_crypto_light(seq_id, algo_name, seq):
    ...
    first_digits = [int(str(abs(x)).replace('.', '').lstrip('0')) for x in seq if abs(x) > 1e-4]
    if first_digits:
...
if __name__ == "__main__":
    print("="*60)
    print("🛸 HASE v1.1-HYPER-HYPE: ГЕНЕРАЦИЯ НЕЛИНЕЙНОЙ ТУРБУЛЕНТНОСТИ")
    print("="*60)
    ...
    for m_type in MARKET_MODELS:
        ...
        for i in range(ROWS_PER_MODEL):
            ...
            # 1. Генерируем временной ряд заданной модели
    ...
    print("\n" + "="*60)
    print(f"👑 СЛОЖНЫЙ НЕЛИНЕЙНЫЙ КРИПТО-МАРКЕТ СГЕНЕРИРОВАН!")
    print(f"📊 Всего сгенерировано: {len(df)} рядов (включая фрактальный хайп).")
